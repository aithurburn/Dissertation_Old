...
The 18-item Dominance, Prestige, and Leadership scale [DoPL; @suessenbach2019], is used to measure dominance, prestige, and leadership orientation. Each question corresponds to one of the three domains. Each domain is scored across six unique items related to those domains (e.g., "I relish opportunities in which I can lead others" for leadership). These statements and goals are rated on a scale from 0 (Strongly disagree) to 5 (Strongly agree). Internal consistency reliability per subscale with the current sample with $\alpha$'s ranging from dominance = `r psych::alpha(newDF[, dominanceQuestions])$total$std.alpha`, prestige = `r psych::alpha(newDF[, prestigeQuestions])$total$std.alpha`, leadership = `r psych::alpha(newDF[, leadershipQuestions])$total$std.alpha`, and UMS = `r psych::alpha(newDF[, UMSQuestions])$total$std.alpha`.
...
apa_table(dm5hdi[
 sign(dm5hdi$CI_low) == sign(dm5hdi$CI_high),
